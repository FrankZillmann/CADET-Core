...
RSMagneticCaptureModel<ConvDispOperator>::RSMagneticCaptureModel(UnitOpIdx unitOpIdx) : UnitOperationBase(unitOpIdx),
...
template <typename ConvDispOperator>
RSMagneticCaptureModel<ConvDispOperator>::~RSMagneticCaptureModel() CADET_NOEXCEPT
{
	delete[] _tempState;
}

template <typename ConvDispOperator> 
unsigned int RSMagneticCaptureModel<ConvDispOperator>::numDofs() const CADET_NOEXCEPT
{
...
template <typename ConvDispOperator>
unsigned int RSMagneticCaptureModel<ConvDispOperator>::numPureDofs() const CADET_NOEXCEPT
{
	// Column bulk DOFs: nCol * nComp mobile phase and nCol * (sum boundStates) solid phase
...
template <typename ConvDispOperator>
bool RSMagneticCaptureModel<ConvDispOperator>::usesAD() const CADET_NOEXCEPT
{
...
template <typename ConvDispOperator>
bool RSMagneticCaptureModel<ConvDispOperator>::configureModelDiscretization(IParameterProvider& paramProvider, const IConfigHelper& helper)
{
	// ==== Read discretization
...
template <typename ConvDispOperator>
bool RSMagneticCaptureModel<ConvDispOperator>::configure(IParameterProvider& paramProvider)
{
	_parameters.clear();
...
template <typename ConvDispOperator>
unsigned int RSMagneticCaptureModel<ConvDispOperator>::threadLocalMemorySize() const CADET_NOEXCEPT
{
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::useAnalyticJacobian(const bool analyticJac)
{
#ifndef CADET_CHECK_ANALYTIC_JACOBIAN
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx, const ConstSimulationState& simState, const AdJacobianParams& adJac)
{
	Indexer idxr(_disc);
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::setFlowRates(active const* in, active const* out) CADET_NOEXCEPT
{
	_convDispOp.setFlowRates(in[0], out[0], _totalPorosity);
}

template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::reportSolution(ISolutionRecorder& recorder, double const* const solution) const
{
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::reportSolutionStructure(ISolutionRecorder& recorder) const
{
...
template <typename ConvDispOperator>
unsigned int RSMagneticCaptureModel<ConvDispOperator>::requiredADdirs() const CADET_NOEXCEPT
{
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::prepareADvectors(const AdJacobianParams& adJac) const
{
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::extractJacobianFromAD(active const* const adRes, unsigned int adDirOffset)
{
	Indexer idxr(_disc);
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::checkAnalyticJacobianAgainstAd(active const* const adRes, unsigned int adDirOffset) const
{
	Indexer idxr(_disc);
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::jacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::residual(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerResidual);
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::residualWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerResidual);
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::residual(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res,
...
int RSMagneticCaptureModel<ConvDispOperator>::residualImpl(double t, unsigned int secIdx, StateType const* const y, double const* const yDot, ResidualType* const res, util::ThreadLocalStorage& threadLocalMem)
{
	if (wantRes)
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::residualSensFwdWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::residualSensFwdAdOnly(const SimulationTime& simTime, const ConstSimulationState& simState, active* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerResidualSens);
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::residualSensFwdCombine(const SimulationTime& simTime, const ConstSimulationState& simState,
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::multiplyWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* yS, double alpha, double beta, double* ret)
{
	Indexer idxr(_disc);
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::multiplyWithDerivativeJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* sDot, double* ret)
{
	Indexer idxr(_disc);
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::setExternalFunctions(IExternalFunction** extFuns, unsigned int size)
{
	if (_binding[0])
...
template <typename ConvDispOperator>
unsigned int RSMagneticCaptureModel<ConvDispOperator>::localOutletComponentIndex(unsigned int port) const CADET_NOEXCEPT
{
...
template <typename ConvDispOperator>
unsigned int RSMagneticCaptureModel<ConvDispOperator>::localInletComponentIndex(unsigned int port) const CADET_NOEXCEPT
{
	return 0;
}

template <typename ConvDispOperator>
unsigned int RSMagneticCaptureModel<ConvDispOperator>::localOutletComponentStride(unsigned int port) const CADET_NOEXCEPT
{
	return 1;
}

template <typename ConvDispOperator>
unsigned int RSMagneticCaptureModel<ConvDispOperator>::localInletComponentStride(unsigned int port) const CADET_NOEXCEPT
{
	return 1;
}

template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::expandErrorTol(double const* errorSpec, unsigned int errorSpecSize, double* expandOut)
{
 ...
 */
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::linearSolve(double t, double alpha, double outerTol, double* const rhs, double const* const weight,
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::assembleDiscretizedJacobian(double alpha, const Indexer& idxr)
{
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::addTimeDerivativeToJacobianCell(linalg::FactorizableBandMatrix::RowIterator& jac, const Indexer& idxr, double alpha, double invBeta) const
{
	// Mobile phase
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::applyInitialCondition(const SimulationState& simState) const
{
	Indexer idxr(_disc);
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::readInitialCondition(IParameterProvider& paramProvider)
{
	_initState.clear();
	_initStateDot.clear();
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::consistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY, double* const vecStateYdot, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
}
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY, double* const vecStateYdot, double* const res, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::initializeSensitivityStates(const std::vector<double*>& vecSensY) const
{
	Indexer idxr(_disc);
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::consistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
 ...
 */
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::leanConsistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
...
template <typename ConvDispOperator>
bool RSMagneticCaptureModel<ConvDispOperator>::setParameter(const ParameterId& pId, double value)
{
...
template <typename ConvDispOperator>
void RSMagneticCaptureModel<ConvDispOperator>::setSensitiveParameterValue(const ParameterId& pId, double value)
{
...
template <typename ConvDispOperator>
bool RSMagneticCaptureModel<ConvDispOperator>::setSensitiveParameter(const ParameterId& pId, unsigned int adDirection, double adValue)
{
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::Exporter::writeMobilePhase(double* buffer) const
{
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::Exporter::writeSolidPhase(double* buffer) const
{
	const int stride = _idx.strideColCell();
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::Exporter::writeSolidPhase(unsigned int parType, double* buffer) const
{
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::Exporter::writeInlet(unsigned int port, double* buffer) const
{
	cadet_assert(port == 0);
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::Exporter::writeInlet(double* buffer) const
{
	std::copy_n(_data, _disc.nComp, buffer);
	return _disc.nComp;
}
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::Exporter::writeOutlet(unsigned int port, double* buffer) const
{
	cadet_assert(port == 0);
...
template <typename ConvDispOperator>
int RSMagneticCaptureModel<ConvDispOperator>::Exporter::writeOutlet(double* buffer) const
{
	if (_model._convDispOp.forwardFlow())
		std::copy_n(&_idx.c(_data, _disc.nCol - 1, 0), _disc.nComp, buffer);
	else
