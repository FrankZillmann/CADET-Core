...
/**
 * @file 
 * Defines the rotor stator magenetic capture model.
 */

#ifndef LIBCADET_RSMAGNETIGCAPTUREMODEL_HPP_
#define LIBCADET_RSMAGNETIGCAPTUREMODEL_HPP_
...
namespace
{
	...
	struct RSMagneticCaptureModelName { };

	template <>
	struct RSMagneticCaptureModelName<cadet::model::parts::AxialConvectionDispersionOperatorBase>
	{
		static const char* identifier() CADET_NOEXCEPT { return "RS_MAGNETIC_CAPTURE_MODEL"; }
	};

	template <>
	struct RSMagneticCaptureModelName<cadet::model::parts::RadialConvectionDispersionOperatorBase>
	{
		static const char* identifier() CADET_NOEXCEPT { return "RADIAL_RS_MAGNETIC_CAPTURE_MODEL"; } // TODO: Check if this is correct
	};
}
...
/** TODO: Add rest of description
 * @brief Rotor stator magnetic capture model
 */
template <typename ConvDispOperator>
class RSMagneticCaptureModel : public UnitOperationBase
{
public:

	RSMagneticCaptureModel(UnitOpIdx unitOpIdx);
	virtual ~RSMagneticCaptureModel() CADET_NOEXCEPT;
	...
	static const char* identifier() CADET_NOEXCEPT { return RSMagneticCaptureModelName<ConvDispOperator>::identifier(); }
...
//	IExternalFunction* _extFun; //!< External function (owned by library user)
	...
	public:
		...
		Exporter(const Discretization& disc, const RSMagneticCaptureModel& model, double const* data) : _disc(disc), _idx(disc), _model(model), _data(data) { }
		Exporter(const Discretization&& disc, const RSMagneticCaptureModel& model, double const* data) = delete;
	...
	protected:
		const Discretization& _disc;
		const Indexer _idx;
		const RSMagneticCaptureModel& _model;
...
#endif  // LIBCADET_RSMAGNETIGCAPTUREMODEL_HPP_
